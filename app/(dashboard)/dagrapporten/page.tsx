...
export default function DagraportenPage() {
  ...
  const copyLink = () => {
    const url = `${window.location.origin}/dagrapporten`
            ...
            <tbody>
              {rapporten.map((r, i) => (
                <tr key={r.id} className={`border-b border-gray-100 hover:bg-gray-50 ${i % 2 === 1 ? 'bg-gray-50/50' : ''}`}>
                  ...
                  <td className="px-4 py-3 text-right font-medium">{r.leads_benaderd}</td>
                  <td className="px-4 py-3 text-right font-medium">{r.cold_calls}</td>
                  <td className="px-4 py-3 text-right font-medium text-[#1A7A3A]">{r.calls_geboekt}</td>
                  <td className="px-4 py-3">
                    {r.op_tijd
                      ? <span className="badge bg-green-100 text-green-700">✓ Ja</span>
                      : <span className="badge bg-red-100 text-red-700">✗ Nee</span>}
                  </td>
                  {isManager && (
                    <td className="px-4 py-3">
              ...
              {rapporten.length === 0 && (
                <tr><td colSpan={8} className="px-4 py-12 text-center text-gray-400">Nog geen dagrapporten</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Dagrapport indienen">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><label className="label">Op tijd ingediend?</label>
